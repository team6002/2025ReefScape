...
public class CMD_ReadyToDeploy extends Command{
    ...
    public CMD_ReadyToDeploy(SUB_Pivot p_pivot, SUB_Elevator p_elevator, SUB_Intake p_intake, SUB_FlippyWrist p_flippyWrist, SUB_SpinnyWrist p_spinnyWrist,
        ...
        addRequirements(m_pivot, m_elevator, m_intake, m_flippyWrist, m_spinnyWrist);
    }
    ...
    public void execute(){
        ...
        if(GlobalVariables.m_targetCoralLevel >= 3){
            ...
            if(GlobalVariables.m_placeFront){
                if(!m_pivot.inPosition(PivotConstants.kDeployFrontL2) && !setPivot){
                    m_pivot.setGoal(PivotConstants.kDeployFrontL2);
                    setPivot = true;
                }
        
                if(!m_elevator.inPosition(ElevatorConstants.kDeployFrontL2) &! setElevator){
                    m_elevator.setGoal(ElevatorConstants.kDeployFrontL2);
                    setElevator = true;
                }
        
                if(!m_flippyWrist.inPosition(FlippyWristConstants.kDeployFrontL2) &! setFlippyWrist){
                    m_flippyWrist.setGoal(FlippyWristConstants.kDeployFrontL2);
                    setFlippyWrist = true;
                }
        
                if(m_flippyWrist.inPosition(FlippyWristConstants.kDeployFrontL2) &! setSpinnyWrist){
                    m_spinnyWrist.setGoal(SpinnyWristConstants.kDeployFront);
                    setSpinnyWrist = true;
                }
            }else{
